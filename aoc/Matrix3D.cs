...
namespace aoc
{
    ...
    public readonly struct Matrix3D : IIntegerMatrix<Matrix3D, Vector3D, int>, IMatrix3D<Matrix3D, Vector3D, int>
    {
        ...
        public static Matrix3D NegativeOne => Helper.NegativeOne;
        public static Matrix3D Zero        => default;
        public static Matrix3D One         => Helper.One;

        public static Matrix3D AdditiveIdentity       => Zero;
        public static Matrix3D MultiplicativeIdentity => One;
        ...
        private static Matrix3D FromRows(ReadOnlySpan<Vector3D> rows) =>
            new(rows[0], rows[1],
                rows.Length > 2 ? rows[2] : default,
                rows.Length > 3 ? rows[3] : default);

        private static Matrix3D FromColumns(ReadOnlySpan<Vector3D> columns) =>
